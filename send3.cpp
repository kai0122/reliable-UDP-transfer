...
int readable_timeout(int fd, int usec){
    fd_set rset;
    struct timeval tv;
...
int main(int argc, char **argv){
    ...
    buffer = (char*) malloc (sizeof(char)*MAXDATA);
    ...
    */
    ...
    fclose(readFile);
    fclose(writeFile);
    //fwrite(sss, 1, MAXDATA*(vec.size()-1)+prev, writeFile); 
    ...
    while(1){
        ...
        if(readable_timeout(sockFd, TIMEOUT) == 0){
            cout << "TOTAL TIME OUT...\n";
            continue;
        }
        else{
